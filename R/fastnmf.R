...
fastnmf<-function(listpart,
  ...
  if(threshold==Inf){
    res<-list(Htilde=NULL,S=NULL,crit=NULL,Mtilde=NULL,cluster=NULL)
    return(res)
  }
  ...
  if(length(listpart.intern)==1){
    ...
    res$best <- res
    return(res)
  }else if(length(listpart.intern)==0){
    res<-list(Htilde=NULL,S=NULL,crit=NULL,Mtilde=NULL,cluster=NULL)
    return(res)
  }
  ...
  res.nmf <- sapply(method.init,FUN=function(method.init,initparts,nb.clust,threshold,printflag,Mtilde){
    ...
    return(res)
  },initparts = res.kmeans,nb.clust=nb.clust,threshold=threshold,printflag=printflag,Mtilde=Mtilde,simplify=FALSE)
